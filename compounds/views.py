...
def compound_detail(request, pk):
    ...
    pubchem_request = requests.get(
        "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchikey/" + compound_InChIKey + "/synonyms/json").json()
...
def compound_data(request):
    compounds = Compound_Data.objects.filter(published_date__lte=timezone.now()).order_by('published_date')
    return render(request, 'compounds/compound_data.html', {'compounds': compounds})
